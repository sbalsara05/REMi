...
export type BorderGlowMode = 'hover' | 'stream';
...
export type BorderGlowProps = {
  ...
  colors?: string[];
  fillOpacity?: number;
  /** When true, runs a rotating edge glow (for text streaming). */
  active?: boolean;
  /** `stream` disables hover; glow follows `active` only. */
  glowMode?: BorderGlowMode;
...
export default function BorderGlow({
  ...
  const hoverEnabled = glowMode === 'hover';
  ...
  const handlePointerMove = useCallback(
    ...
    [getEdgeProximity, getCursorAngle, hoverEnabled],
  );
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      card.classList.remove('stream-active');
      card.style.setProperty('--edge-proximity', '0');
    };
  }, [active]);

  useEffect(() => {
    if (!animated || !cardRef.current || active) {
      return undefined;
    }
    const card = cardRef.current;
  ...
  return (
    <div
      ...
      className={`border-glow-card ${hoverEnabled ? '' : 'border-glow-card--stream'} ${className}`}
      style={style}
